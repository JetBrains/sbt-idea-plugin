...
class JbIdeaRepoArtifactResolverTest extends FunSuite with Matchers with IdeaMock with ConsoleLogger {
  ...
  test("latest eap is resolved") {
    ...
    val result = resolver.resolveUrlForIdeaBuild(BuildInfo("LATEST-EAP-SNAPSHOT", IntelliJPlatform.IdeaCommunity))
    result should not be empty
  }

  test("latest 192 release is resolved") {
    val resolver = createResolver
    val result = resolver.resolveUrlForIdeaBuild(BuildInfo("192.6603.28", IntelliJPlatform.IdeaCommunity))
    result should not be empty
  }

  test("latest 191 release is resolved") {
    val resolver = createResolver
    val result = resolver.resolveUrlForIdeaBuild(BuildInfo("191.7479.19", IntelliJPlatform.IdeaCommunity))
    result should not be empty
  }
