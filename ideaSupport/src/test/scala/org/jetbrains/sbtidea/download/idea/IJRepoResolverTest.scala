...
class IJRepoResolverTest extends AnyFunSuite with Matchers with IdeaMock with TmpDirUtils {
  ...
  test("IJ repo overriding property") {
    try {
      ...
      messages should contain("[warn] [IntellijVersionUtils] Cannot detect artifact location for version 261.22158.277, fallback to: (261.22158-EAP-CANDIDATE-SNAPSHOT,intellij-repository-eap: https://127.0.0.1:/non-existing-path/snapshots)")
      messages should contain("[warn] [IntellijVersionUtils] Cannot detect artifact location for version 261.22158.277, fallback to: (261.22158-EAP-CANDIDATE-SNAPSHOT,intellij-repository-eap: https://127.0.0.1:/non-existing-path/snapshots)")
    } finally {
