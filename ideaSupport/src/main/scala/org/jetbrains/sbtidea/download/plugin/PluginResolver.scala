...
class PluginResolver(private val processedPlugins: Set[IntellijPlugin] = Set.empty)
  ...
  private def resolveDependencies(plugin: PluginDependency, key: IntellijPlugin, descriptor: PluginDescriptor) = {
    descriptor.dependsOn
      ...
      .flatMap(new PluginResolver(processedPlugins + key).resolve)
  }
    ...
    if (!localRegistry.isPluginInstalled(key)) {
      ...
      descriptor match {
        case Right(value) =>
          ...
          val resolvedDeps = resolveDependencies(plugin, key, value)
          thisPluginArtifact +: resolvedDeps
        case Left(error) =>
